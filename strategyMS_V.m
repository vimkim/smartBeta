function portfolio=strategyMS_V(thisCrsp, marketSigma, mw, sw, vw)
    ...
    port1 = thisCrsp(thisCrsp.momentumRank >= mw(1) & thisCrsp.momentumRank <= mw(2), :);
    port1.sizeRank(:) = NaN;
    port1 = addRank('size', port1);
    port1 = port1(port1.sizeRank >= sw(1) & port1.sizeRank <= sw(2), :);
    port2 = thisCrsp(thisCrsp.valueRank >= vw(1) & thisCrsp.valueRank <= vw(2), :);
